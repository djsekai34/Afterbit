...
export default function Barra({ isDark, setIsDark, isFormPage }) {
  ...
  // Filtrado dinámico basado en etiquetas y etiquetas del mapa web
  const sugerencias =
    searchQuery.trim() === ""
      ? []
      : mapaWeb
  ...
  // Control de colisión del submenú para evitar que se salga de la pantalla
  ...
  useEffect(() => {
    // Cerrar menús al hacer clic fuera de los contenedores (búsqueda y menú móvil)
  ...
  useEffect(() => {
    // Sincronización del color de fondo global con el estado Dark Mode
  ...
  // Determinamos si estamos específicamente en la página del formulario para el logo
  const isStrictForm = window.location.pathname === "/FormularioTesteo";
  ...
  return (
    ...
    >
      {/* CONTENEDOR PRINCIPAL: Ajusta el flujo si es página de formulario (centrado) o navegación normal (espaciado) */}
      <div className={`container mx-auto px-6 lg:px-8 py-4 flex items-center h-[95px] ${isFormPage ? "justify-center gap-12" : "justify-between"}`}>
        
        {/* SECCIÓN DEL LOGO: Lógica condicional para desactivar navegación en el formulario */}
        <div className="flex-shrink-0">
          {isStrictForm ? (
            <div className="flex-shrink-0">
              <img
                src={AfterbitLogo}
                alt="Afterbit logo static"
                className="h-28 sm:h-32 md:h-35 lg:h-36 w-auto mt-3 cursor-default opacity-90"
              />
            </div>
          ) : (
        ...
        {/* NÚCLEO DE NAVEGACIÓN: Alterna entre el botón simplificado (Formulario) y el Menú Completo */}
        {isFormPage ? (
          /* VISTA FORMULARIO: Solo Switch de Modo Oscuro */
          <div className="flex items-center">
            <button
              onClick={() => toggleDarkMode()}
              className={`py-2 px-6 font-black uppercase italic tracking-tighter border-2 transition-all transform hover:scale-110 active:scale-95 text-xs xl:text-sm whitespace-nowrap ${
          ...
          /* VISTA GENERAL: Buscador, Links y Menú Desplegable */
          <>
            <ul
              className={`flex flex-col lg:flex-row items-center lg:gap-4 xl:gap-10 absolute lg:static left-0 w-full lg:w-auto transition-all duration-500 ease-in-out
            ...
            >
              ...
              <li className="relative py-2 lg:py-0 lg:mr-1 xl:mr-6" ref={searchRef}>
                <div
                  className={`flex items-center group border-b-2 transition-all duration-300 w-[140px] lg:w-[130px] xl:w-[200px] xl:focus-within:w-[240px] 
                ...
                {showSuggestions && sugerencias.length > 0 && (
                  <ul className={`absolute top-[110%] left-1/2 -translate-x-1/2 lg:left-0 lg:translate-x-0 w-[90vw] lg:w-64 mt-2 border-2 shadow-[0_25px_60px_rgba(0,0,0,0.4)] z-[120] backdrop-blur-md ${isDark ? "bg-black/95 border-white" : "bg-white/95 border-black"}`}>
                    {sugerencias.map((s, i) => (
                      <li key={i} className={`border-b last:border-0 ${isDark ? "border-white/10" : "border-black/10"}`}>
                        <button
                          ...
                          className={`w-full text-left p-4 flex items-center transition-all group/item ${isDark ? "hover:bg-white" : "hover:bg-black"}`}
                        >
              ...
              {/* Menú de Proyectos con Sub-navegación Multinivel */}
              <li className="relative group py-2">
                <Link to="/Proyectos" onClick={closeMenu} className="group">
                  <button className={`font-bold uppercase tracking-widest text-sm flex items-center gap-2 whitespace-nowrap ${isDark ? "text-white" : "text-black"}`}>
                    Proyectos
                    <span className="text-[12px] group-hover:rotate-180 transition-transform duration-300 pointer-events-none hidden lg:block [@media(pointer:coarse)]:!hidden">
                      ↓
                    </span>
                  </button>
                </Link>
                {/* Primer Nivel: Lista de Proyectos */}
                <ul className="absolute left-1/2 -translate-x-1/2 lg:left-0 lg:translate-x-0 top-[100%] pt-2 min-w-[350px] invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col z-[110]">
                  {mapaWeb
                    ...
                    .map((proyecto, index) => {
                      ...
                      return (
                        <li
                          key={proyecto.path + index}
                          ref={esSuperRodolfoPrincipal ? rodolfoRef : null}
                          onMouseEnter={esSuperRodolfoPrincipal ? handleMouseEnter : null}
                        ...
                        >
                          ...
                          {/* Segundo Nivel: Sub-links específicos de Super Rodolfo */}
                          {esSuperRodolfoPrincipal && (
                            <ul className={`absolute top-[-2px] ${subMenuDirection} min-w-[280px] max-h-[80vh] overflow-y-auto invisible group-hover/item-proyecto:visible opacity-0 group-hover/item-proyecto:opacity-100 transition-all border-2 ${isDark ? "border-white bg-black" : "border-black bg-white"}`}>
                              {mapaWeb
                                .filter((subItem) => subItem.path.startsWith("/Proyectos/SuperRodolfo/"))
                                .map((p, idx) => (
            ...
            {/* CONTROL MÓVIL: Botón Hamburguesa*/}
