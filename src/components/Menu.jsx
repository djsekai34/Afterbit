...
export default function Barra({ isDark, setIsDark }) {
  ...
  const sugerencias = searchQuery.trim() === "" 
    ...
    : mapaWeb.filter(item => {
        ...
        return item.label.toLowerCase().includes(query) || item.tags.some(tag => tag.toLowerCase().includes(query));
      }).slice(0, 6);
  ...
  const handleMouseEnter = () => {
    if (rodolfoRef.current) {
      ...
      setSubMenuDirection(window.innerWidth - rect.right < 300 ? "right-full mr-[2px]" : "left-full ml-[2px]");
    }
  };
        ...
        <ul className={`flex flex-col lg:flex-row items-center lg:gap-10 absolute lg:static left-0 w-full lg:w-auto transition-all duration-500 ease-in-out
          ...
          <li className="relative py-2 lg:py-0 lg:mr-4" ref={searchRef}>
            <div className={`flex items-center group border-b-2 transition-all duration-300 ${
              ...
              <span className={`font-mono text-[9px] font-bold px-1 transition-opacity ${searchQuery ? "opacity-100" : "opacity-30"} ${isDark ? "text-white" : "text-black"}`}>
                {">"}
              </span>
              <input
                type="text"
            ...
            {showSuggestions && sugerencias.length > 0 && (
              <ul className={`absolute top-full left-0 w-64 mt-3 border-2 shadow-[0_20px_50px_rgba(0,0,0,0.3)] z-[120] backdrop-blur-sm ${
                isDark ? "bg-black/95 border-white" : "bg-white/95 border-black"
              }`}>
                {sugerencias.map((s, i) => (
                  <li key={i} className="border-b border-zinc-500/10 last:border-0">
                    <button 
                    ...
                    >
                      <span className={`font-black text-[9px] tracking-tighter uppercase transition-colors ${
                        isDark ? "text-white group-hover/item:text-black" : "text-black group-hover/item:text-white"
                      }`}>
                        {s.label}
                      </span>
                      <div className="flex flex-wrap gap-1 mt-0.5">
                        {s.tags.map((tag, index) => (
                          <span key={index} className={`text-[10px] font-bold font-mono px-1 border uppercase tracking-tighter transition-colors ${
                            isDark 
          ...
          <li className="relative group py-2">
            ...
            <ul className="absolute left-1/2 -translate-x-1/2 lg:left-0 lg:translate-x-0 top-[100%] pt-2 min-w-[320px] invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col z-[110]">
              ...
              <li className={`border-2 border-t-0 ${isDark ? "border-white bg-black" : "border-black bg-white"}`}>
                <Link to="/Proyectos/SuperRodolfoStrike" className={dropdownItemStyles} onClick={closeMenu}>Super Rodolfo Strike</Link>
              </li>
              <li className={`border-2 border-t-0 ${isDark ? "border-white bg-black" : "border-black bg-white"}`}>
                <Link to="/Proyectos/PixelMetroid2D" className={dropdownItemStyles} onClick={closeMenu}>Pixel Metroid 2D</Link>
              </li>
              <li className={`border-2 border-t-0 ${isDark ? "border-white bg-black" : "border-black bg-white"}`}>
                <Link to="/Proyectos/XRAdventure" className={dropdownItemStyles} onClick={closeMenu}>XR Adventure</Link>
              </li>
            </ul>
          </li>
