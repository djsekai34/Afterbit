...
export default function Barra({ isDark, setIsDark }) {
  ...
  return (
    ...
    ${isDark ? "border-white/30 focus-within:border-white shadow-[0_1px_0_0_rgba(255,255,255,0.1)]" : "border-black/30 focus-within:border-black shadow-[0_1px_0_0_rgba(0,0,0,0.05)]"}`}
            >
              <span
                className={`font-mono text-[12px] font-black px-2 transition-all duration-300 ${searchQuery || showSuggestions ? "opacity-100 scale-110" : "opacity-50"} ${isDark ? "text-white" : "text-black"}`}
              >
                {">"}
              </span>
              <input
            ...
            {showSuggestions && sugerencias.length > 0 && (
              <ul
                className={`absolute top-[110%] left-1/2 -translate-x-1/2 lg:left-0 lg:translate-x-0 w-[90vw] lg:w-64 mt-2 border-2 shadow-[0_25px_60px_rgba(0,0,0,0.4)] z-[120] backdrop-blur-md ${isDark ? "bg-black/95 border-white" : "bg-white/95 border-black"}`}
              >
                {sugerencias.map((s, i) => (
                  <li
                    key={i}
                    className={`border-b last:border-0 ${isDark ? "border-white/10" : "border-black/10"}`}
                  >
                    <button
                      onClick={() => {
                        navigate(s.path);
                    ...
                    >
                      <span
                        className={`font-black text-[10px] tracking-wider uppercase transition-colors ${isDark ? "text-white group-hover/item:text-black" : "text-black group-hover/item:text-white"}`}
                      >
                        {s.label}
                      </span>
                    </button>
          ...
          <li className="relative group py-2">
            <Link to="/Proyectos" onClick={closeMenu} className="group">
              <button
                className={`font-bold uppercase tracking-widest text-sm flex items-center gap-2 whitespace-nowrap ${isDark ? "text-white" : "text-black"}`}
              >
                Proyectos
            ...
            <ul className="absolute left-1/2 -translate-x-1/2 lg:left-0 lg:translate-x-0 top-[100%] pt-2 min-w-[350px] invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col z-[110]">
              {mapaWeb
                .filter(
                  (item) =>
                    item.path.startsWith("/Proyectos/") &&
                    item.path.split("/").length === 3,
                )
                .map((proyecto, index) => {
                  const esSuperRodolfoPrincipal =
                    proyecto.path === "/Proyectos/SuperRodolfo";
                  return (
                    <li
                      key={proyecto.path + index}
                      ref={esSuperRodolfoPrincipal ? rodolfoRef : null}
                      onMouseEnter={
                        esSuperRodolfoPrincipal ? handleMouseEnter : null
                      }
                    ...
                    >
                      ...
                      {esSuperRodolfoPrincipal && (
                        <ul
                          className={`absolute top-[-2px] ${subMenuDirection} min-w-[280px] max-h-[80vh] overflow-y-auto invisible group-hover/item-proyecto:visible opacity-0 group-hover/item-proyecto:opacity-100 transition-all border-2 ${isDark ? "border-white bg-black" : "border-black bg-white"}`}
                        >
                          {mapaWeb
                            .filter((subItem) =>
                              subItem.path.startsWith(
                                "/Proyectos/SuperRodolfo/",
                              ),
                            )
                            .map((p, idx) => (
                              <li
          ...
          {mapaWeb
            .filter((item) =>
              ["/Eventos", "/SobreMi", "/Contacto"].includes(item.path),
            )
            .map((seccion) => (
              <li key={seccion.path}>
                <Link
