...
export default function Eventos({ isDark }) {
  ...
  const downloadIcsFile = (event) => {
    ...
    const startDate = event.especificos
      ? new Date(event.especificos[0])
  ...
  const getVisibleColor = (hexColor) => {
    ...
    if (isDark) {
      if (brightness < 60)
        return `rgb(${Math.min(r + 90, 255)}, ${Math.min(g + 90, 255)}, ${Math.min(b + 120, 255)})`;
    } else {
      if (brightness > 200)
        return `rgb(${Math.max(r - 100, 0)}, ${Math.max(g - 100, 0)}, ${Math.max(b - 100, 0)})`;
    }
    return hexColor;
  };
  ...
  const formatEventDate = (event) => {
    ...
    if (event.especificos)
      return event.especificos.map((t) => f(new Date(t))).join(" , ");
  ...
  const currentMonthEvents = useMemo(() => {
    ...
    return eventsData.filter((e) => {
      const isThisMonth = e.especificos
        ? e.especificos.some(
            (t) =>
              new Date(t).getMonth() === month &&
              new Date(t).getFullYear() === year,
          )
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8">
          <div
            className={`relative border-2 ${isDark ? "border-zinc-800 bg-zinc-900/20" : "border-zinc-200 bg-zinc-50"} p-6 shadow-2xl`}
          >
            {/* CONTROLES DE NAVEGACIÓN */}
            <div className="flex justify-between items-center mb-8">
              <button
                onClick={() => changeMonth(-1)}
                className="hover:scale-125 transition-transform p-2 font-black text-xl"
              >
                {"<<"}
              </button>
                  ...
                  {showSelector && (
                    ...
                    >
                      <div className="flex justify-between items-center mb-4 pb-2 border-b border-zinc-500/20">
                        <p className="text-[9px] opacity-50 font-mono font-bold uppercase tracking-widest">
                          // SELECCIÓN_FECHA
                        </p>
                        <button
                          onClick={jumpToToday}
                      ...
                      <div className="grid grid-cols-2 gap-8 mb-6">
                        <div>
                          <p className="text-[10px] opacity-50 mb-4 font-mono">
                            // MES
                          </p>
                      ...
                      <button
                        onClick={() =>
                          jumpToDate(tempSelection.month, tempSelection.year)
                        }
            ...
            <div className="grid grid-cols-7 border-t border-l border-zinc-500/20">
              {weekDays.map((day) => (
                <div
                  key={day}
                  className="p-2 sm:p-4 text-center font-mono text-[10px] opacity-40 border-r border-b border-zinc-500/20"
                >
                  {day}
                </div>
              ))}
              {daysInMonth.map((date, index) => {
                ...
                const visualColor = event
                  ? getVisibleColor(event.color)
                  : "transparent";
                const isSelected =
                  ...
                  >
                    <AnimatePresence>
                      {isHovered && event && (
                        ...
                        >
                          <div
                            className={`flex items-center gap-2 px-3 py-1.5 backdrop-blur-md border shadow-2xl ${isDark ? "bg-zinc-900/90 border-zinc-700 text-white" : "bg-white/95 border-zinc-300 text-black"}`}
                          >
                            <div
                              className="w-1 h-3"
                    ...
                    {date && (
                      <span
                        className={`font-mono text-xs ${event ? "font-black" : "opacity-30"}`}
                      >
                        {date.getDate()}
                      </span>
                    )}
                    {event && (
                      <div
                        className="absolute left-0 right-0 h-1.5 sm:h-2 bottom-4 opacity-80"
                        style={{
                          backgroundColor: visualColor,
                          boxShadow: isDark
                            ? `0 0 15px ${visualColor}66`
                            : "none",
                        }}
                      />
                    )}
              ...
              {currentMonthEvents.length > 0 ? (
                currentMonthEvents.map((e) => (
                  <div key={e.id} className="flex items-center gap-2">
                    <div
                      className="w-3 h-3"
                      style={{ backgroundColor: getVisibleColor(e.color) }}
                    ></div>
        ...
        <div className="lg:col-span-4">
          <div className="sticky top-32">
            <h3 className="font-mono text-xs opacity-50 mb-4 uppercase tracking-[0.3em]">
              // EVENT_LOG
            </h3>
            <AnimatePresence mode="wait">
              {selectedEvent ? (
                <Motion.div
                  ...
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: -20 }}
                  className={`border-2 p-6 shadow-2xl ${isDark ? "border-zinc-700 bg-zinc-900" : "border-black bg-zinc-50"}`}
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-2">
                      <div
                        ...
                        style={{
                          backgroundColor: getVisibleColor(selectedEvent.color),
                          boxShadow: `0 0 8px ${getVisibleColor(selectedEvent.color)}`,
                        }}
                      ></div>
                    ...
                    <button
                      ...
                      style={{ color: getVisibleColor(selectedEvent.color) }}
                    >
                      [CERRAR]
                    </button>
                  </div>

                  <h4 className="text-2xl font-black italic uppercase mb-6 tracking-tighter leading-tight">
                    {selectedEvent.titulo}
                  </h4>

                  <div className="space-y-6 font-mono text-[11px]">
                    <p className="opacity-70 leading-relaxed border-l-2 border-zinc-500/30 pl-4">
                    ...
                    <div className="space-y-4 pt-4">
                      <div>
                        <span
                          style={{
                            color: getVisibleColor(selectedEvent.color),
                          }}
                        >
                          [ FECHA ]
                        </span>
                      ...
                      <div>
                        ...
                        {selectedEvent.url ? (
                          <a
                            ...
                            className="opacity-90 hover:underline inline-flex items-center gap-1 group uppercase"
                          >
                            {selectedEvent.ubicacion}{" "}
                            <span className="text-[8px] group-hover:translate-x-1 transition-transform">
                              ↗
                            </span>
                          </a>
                        ) : (
                    ...
                    {(selectedEvent.fin || selectedEvent.inicio) >=
                ...
                <div
                  className={`border-2 border-dashed p-12 text-center opacity-20 ${isDark ? "border-white" : "border-black"}`}
                >
