        ...
        >
          ...
          <div className="flex flex-col xl:flex-row gap-10 items-start max-w-7xl mx-auto">
              ...
              >
                <div className="flex justify-between items-start mb-6">
                  <div className="flex flex-col">
                    <span className={`text-[8px] font-black tracking-[0.3em] uppercase ${isDark ? "text-zinc-500" : "text-zinc-400"}`}>
                  ...
                  <div className={`w-10 h-10 border-2 rounded-full hidden md:flex items-center justify-center rotate-12 ${isDark ? "border-zinc-700 text-zinc-700" : "border-zinc-300 text-zinc-300"}`}>
                ...
                <div className="flex flex-col gap-1">
                  <h3 className={`text-xl md:text-2xl font-black italic tracking-tighter uppercase leading-none ${isDark ? "text-white" : "text-black"}`}>
                    Tracklist <span className="text-[#008012]">Oficial</span>
                  </h3>
                  <p className={`text-[10px] font-bold tracking-[0.1em] uppercase opacity-70 ${isDark ? "text-zinc-400" : "text-zinc-600"}`}>
                    Super Rodolfo y las esferas del Santo Reino
                  </p>
                </div>

                <div className={`mt-8 flex items-end gap-1 opacity-40 ${isDark ? "invert" : ""}`}>
                  {[2, 4, 1, 3, 2, 6, 1, 4, 2].map((w, i) => (
                    <div key={i} className="bg-black" style={{ width: `${w}px`, height: "15px" }}></div>
                  ))}
                  <span className="text-[8px] ml-2 font-mono tracking-tighter">8 4012026 00007 2</span>
                </div>
                <div className={`absolute bottom-0 left-0 w-full h-[1px] ${isDark ? "bg-white/10" : "bg-black/10"}`}></div>
                <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none ${isDark ? "bg-white/5" : "bg-black/5"}`} />
              </div>
              ...
              >
                {playlist.map((track, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className={`absolute left-0 top-0 h-full w-1 transition-transform duration-300 ${isActive ? "bg-[#008012]" : "bg-zinc-500 -translate-x-full group-hover:translate-x-0"}`} />
                      <div className="flex flex-col items-center">
                        <span className={`font-mono text-[10px] leading-none mb-1 ${isActive ? "opacity-100" : "opacity-30"}`}>TRK</span>
                        <span className={`font-black text-lg font-mono leading-none ${isActive ? "text-[#008012]" : "opacity-20"}`}>
                          {(index + 1).toString().padStart(2, "0")}
                        </span>
                      </div>
                      <div className="flex-grow">
                        <h4 className={`font-black text-[11px] md:text-xs uppercase tracking-[0.15em] leading-tight ${isActive ? "" : "group-hover:text-[#008012] transition-colors"}`}>
                      ...
                      {isActive && (
                        <div className="flex gap-0.5 items-end h-3">
                          <div className={`w-[2px] h-full bg-[#008012] animate-[pulse_0.8s_infinite]`} />
                          <div className={`w-[2px] h-2 bg-[#008012] animate-[pulse_1.2s_infinite]`} />
                          <div className={`w-[2px] h-3 bg-[#008012] animate-[pulse_1s_infinite]`} />
                        </div>
                      )}
                    </button>
                  );
            ...
            >
                ...
                <div className="relative group shrink-0">
                  ...
                  >
                    <div className="absolute inset-0 opacity-20 pointer-events-none"
                      style={{ background: "conic-gradient(from 0deg, transparent 0%, white 15%, transparent 30%, transparent 50%, white 65%, transparent 80%, transparent 100%)" }}
                    />
                    <div className="w-16 h-16 md:w-20 md:h-20 rounded-full border-2 border-white/40 z-10 bg-zinc-900 flex items-center justify-center overflow-hidden">
                      {currentTrack.cover ? (
                        ...
                        <span className="text-2xl font-black italic text-white">{currentTrack.title[0]}</span>
                      )}
                    </div>
                  </div>
                  {/* Aguja del tocadiscos */}
                  <div className={`absolute -top-4 -right-4 w-1 h-32 bg-zinc-400 origin-top transition-transform duration-700 z-20 ${isPlaying ? "rotate-[25deg]" : "rotate-0"}`}>
                    <div className="absolute bottom-0 -left-1 w-3 h-6 bg-zinc-500 rounded-sm shadow-xl"></div>
                  </div>
                </div>

                <div className="flex-grow text-center md:text-left">
                  <span className="text-white px-2 py-0.5 text-[10px] font-black uppercase tracking-widest" style={{ backgroundColor: accentGreen }}>
                    Reproduciendo
                  </span>
                  <h2 className="text-3xl md:text-4xl font-black uppercase mt-2 leading-none">
                  ...
                  <p className="font-mono text-sm mt-4 opacity-70 italic">{currentTrack.desc}</p>
                </div>
              </div>

              {/* CONTROLES DE REPRODUCCIÓN */}
              <div className={`p-6 border-2 ${isDark ? "border-white" : "border-black shadow-[8px_8px_0_0_#000]"}`}>
                <div className="flex justify-between items-end mb-2 font-mono text-[10px] font-black">
                  <span>TRACK_{(currentTrackIndex + 1).toString().padStart(2, "0")}</span>
                  <span style={{ color: accentGreen }}>{currentTimeLabel} - {durationLabel}</span>
                </div>
                ...
                <div className="relative w-full h-10 bg-zinc-500/10 mb-6 flex items-center overflow-hidden">
                  ...
                  />
                  <div className="absolute h-full left-0 bg-zinc-500/20"
                    style={{ width: `${progress}%`, borderRight: `2px solid ${accentGreen}` }}
                  />
                ...
                <div className="flex items-center gap-8">
                  ...
                  <button onClick={handleReset} className="hover:rotate-[-90deg] transition-transform">
                    <RotateCcw size={28} />
                  </button>
                  ...
                  <div className="flex items-center gap-3 flex-grow max-w-[150px]">
                    <button onClick={toggleMute} className="shrink-0 hover:scale-110">
              ...
              <div className={`p-6 border-2 w-full ${isDark ? "border-white/10" : "border-black/5"}`}>
                <h5 className="font-black text-[10px] uppercase mb-2" style={{ color: accentGreen }}>
            ...
            <div className="flex flex-col gap-10 opacity-50 hover:opacity-100 transition-opacity duration-700">
              <div className="flex flex-col gap-4 text-center md:text-left">
                <p className="font-mono text-[10px] md:text-[11px] tracking-[0.15em] leading-relaxed uppercase">
                  Este <span className={isDark ? "text-white" : "text-black font-bold"}>videojuego y página web</span> es un proyecto académico sin fines de lucro, 
                  desarrollado por <span className={isDark ? "text-white font-black" : "text-black font-black"}>David Jiménez Villena</span> como pieza de 
                  portfolio personal dentro del <span className="italic">Curso de Especialización en Desarrollo de Videojuegos y Realidad Virtual</span>.
                </p>
                <p className="font-mono text-[10px] md:text-[11px] tracking-[0.15em] leading-relaxed uppercase">
                  El objetivo de este proyecto es demostrar competencias técnicas en programación, diseño de niveles, 
                  integración en el motor <span className={isDark ? "text-white" : "text-black font-bold"}>Unity</span>, 
                  así como el <span className={isDark ? "text-white font-bold" : "text-black font-bold"}>desarrollo y gestión de un GDD</span> profesional de un videojuego.
                </p>
              </div>

              <div className={`p-6 border-x-2 border-y font-mono ${isDark ? "bg-zinc-900/30 border-white/5" : "bg-zinc-100/50 border-black/5"}`}>
                <p className="text-[9px] md:text-[10px] tracking-[0.2em] leading-loose uppercase text-center max-w-5xl mx-auto">
                  Los personajes, música y recursos artísticos pertenecientes a la franquicia 
                  <span className={isDark ? "text-zinc-300" : "text-zinc-700 font-bold"}> Dragon Ball (incluyendo sus etapas original, Z y Super) </span> 
                  son propiedad intelectual de Akira Toriyama, Bird Studio, Shueisha y Toei Animation. 
                  Estos elementos se utilizan aquí de forma excepcional y exclusivamente con fines demostrativos 
                  de desarrollo técnico y aprendizaje, <span className="font-black underline decoration-[#008012]">sin ninguna intención de explotación comercial</span>.
                </p>
              </div>
            </div>
          </Motion.footer>
